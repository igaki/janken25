...
- 実装を開始する前に自分がmainブランチにいることを確認すること．mainブランチにいない場合はmainブランチに移動するようユーザに指示すること
  - `git checkout main` または `git switch main` を使用するよう指示すること
- mainブランチにいることが確認出来たら，`git switch`を用いて新規実装のための新しいブランチを作成するようユーザに指示すること
  - ブランチ名の例：feat/機能名）
...
- `docs/specs.md` と  `docs/requirements.md` と `docs/tasks.md` を読み込み，実装後のコードベースについての説明が各mdファイルに網羅的に記述されていることをまず確認すること
...
| `docs/reports/done_*.md` | 実装等の履歴。実装フェーズで作成 |
| `docs/reports/review_*.md` | レビュー結果の履歴。レビューフェーズで作成 |
| `docs/requirements.md` | 現状の要求仕様。実装済みの機能に対する要求事項を記載。実装フェーズ及びレビューフェーズで更新 |
| `docs/specs.md` | 現状のシステム仕様。システム全体の構成と実装済みの機能を記載。実装フェーズ及びレビューフェーズで更新 |
| `docs/tasks.md` | 現在の作業計画。計画フェーズで作成・更新 |
| `docs/tasks_done.md` | 完了した作業履歴。実装フェーズで更新 |
